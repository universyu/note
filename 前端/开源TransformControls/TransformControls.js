...
const _tempVector2 = new Vector3(1, 1, 1)
...
class TransformControls extends Object3D {
  prePosition = [0, 0, 0]
  preOffsetZ = 0
  constructor(camera, domElement, uiConfig, controlFunction) {
    super()
    this.editStates = {
      isLockedScale: false,
    }
    ...
    const _gizmo = new TransformControlsGizmo(uiConfig)
    this._gizmo = _gizmo
    this.add(_gizmo)
  ...
  changeEditStates(obj) {
    for (const key in obj) {
      if (Object.prototype.hasOwnProperty.call(this.editStates, key)) {
        this.editStates[key] = obj[key]
      }
    }
  }
  ...
  pointerDown(pointer) {
    ...
    if (this.controlFunction?.pointerDown) {
      this.controlFunction.pointerDown(this.object.position, this._offset)
    }
  }
  ...
  pointerMove(pointer) {
    ...
    if (mode === 'translate') {
      ...
      let exceTranslate
      if (this.controlFunction?.preTranslate) {
        exceTranslate = this.controlFunction.preTranslate(this._offset, this._positionStart, axis)
      }
      if (exceTranslate) {
      ...
      if (axis.search('XYZ') !== -1) {
        ...
        let exceScale
        if (this.controlFunction?.preScale) {
          exceScale = this.controlFunction.preScale(_tempVector, _tempVector2, axis)
        }
        if (exceScale) {
          if (axis.search('X') === -1) {
            _tempVector2.x = 1
      ...
      if (space === 'local' && axis !== 'E' && axis !== 'XYZE') {
        ...
        if (this.controlFunction?.rotate) {
          this.controlFunction.rotate(axis)
        }
      }
    }
  ...
  pointerUp(pointer) {
    ...
    this.dragging = false
    this.axis = null
    if (this.controlFunction?.pointerUp && this.object) {
      this.controlFunction.pointerUp(
...
class TransformControlsGizmo extends Object3D {
  constructor(uiConfig) {
    ...
    const colorMappings = uiConfig.colorMappings || {
      red: 0xe14747,
      green: 0xb6f34f,
      blue: 0x0e65e9,
    }
    ...
    matHelper.opacity = uiConfig.matHelperOpacity || 0.5

    const matRed = gizmoMaterial.clone()
    matRed.color.setHex(colorMappings.red)

    const matGreen = gizmoMaterial.clone()
    matGreen.color.setHex(colorMappings.green)

    const matBlue = gizmoMaterial.clone()
    matBlue.color.setHex(colorMappings.blue)
    ...
    const createGizmoTranslateAxis = (axisConfigItem) => {
      let mesh
      if (axisConfigItem[0] === 'arrow') {
        mesh = arrowGeometry
      } else if (axisConfigItem[0] === 'line') {
        mesh = lineGeometry2
      }

      let meshColor
      if (axisConfigItem[1] === 'r') {
        meshColor = matRed
      } else if (axisConfigItem[1] === 'g') {
        meshColor = matGreen
      } else if (axisConfigItem[1] === 'b') {
        meshColor = matBlue
      }

      return [new Mesh(mesh, meshColor), axisConfigItem[2], axisConfigItem[3]]
    }
    const uiGzConfig = uiConfig.gzConfig
    const uiTranslateConfig = uiGzConfig?.translate
    let gizmoTranslate = {}
    if (uiTranslateConfig) {
      if (uiTranslateConfig.x) {
        gizmoTranslate.X = []
        uiTranslateConfig.x.forEach((item) => {
          const newConfig = createGizmoTranslateAxis(item)
          gizmoTranslate.X.push(newConfig)
        })
      }
      if (uiTranslateConfig.y) {
        gizmoTranslate.Y = []
        uiTranslateConfig.y.forEach((item) => {
          const newConfig = createGizmoTranslateAxis(item)
          gizmoTranslate.Y.push(newConfig)
        })
      }
      if (uiTranslateConfig.z) {
        gizmoTranslate.Z = []
        uiTranslateConfig.z.forEach((item) => {
          const newConfig = createGizmoTranslateAxis(item)
          gizmoTranslate.Z.push(newConfig)
        })
      }
      if (uiTranslateConfig.xyz) {
        gizmoTranslate.XYZ = [
    ...
    const uiRotateConfig = uiGzConfig?.rotate
    let gizmoRotate = {}
    if (uiRotateConfig) {
      if (uiRotateConfig.xyze) {
        gizmoRotate.XYZE = [[new Mesh(CircleGeometry(0.5, 1), matGray), null, [0, Math.PI / 2, 0]]]
      }
      if (uiRotateConfig.x) {
        gizmoRotate.X = [[new Mesh(CircleGeometry(0.5, 0.5), matRed)]]
      }
      if (uiRotateConfig.y) {
        gizmoRotate.Y = [[new Mesh(CircleGeometry(0.5, 0.5), matGreen), null, [0, 0, -Math.PI / 2]]]
      }
      if (uiRotateConfig.z) {
        gizmoRotate.Z = [[new Mesh(CircleGeometry(0.5, 0.5), matBlue), null, [0, Math.PI / 2, 0]]]
      }
      if (uiRotateConfig.e) {
        gizmoRotate.E = [
    ...
    const createGizmoScaleAxis = (axisConfigItem) => {
      let mesh
      if (axisConfigItem[0] === 'handle') {
        mesh = scaleHandleGeometry
      } else if (axisConfigItem[0] === 'line') {
        mesh = lineGeometry2
      }
    ...
    const uiScaleConfig = uiGzConfig?.scale
    let gizmoScale = {}
    if (uiScaleConfig) {
      if (uiScaleConfig.x) {
        gizmoScale.X = []
        uiScaleConfig.x.forEach((item) => {
          const newConfig = createGizmoScaleAxis(item)
          gizmoScale.X.push(newConfig)
        })
      }
      if (uiScaleConfig.y) {
        gizmoScale.Y = []
        uiScaleConfig.y.forEach((item) => {
          const newConfig = createGizmoScaleAxis(item)
          gizmoScale.Y.push(newConfig)
        })
      }
      if (uiScaleConfig.z) {
        gizmoScale.Z = []
        uiScaleConfig.z.forEach((item) => {
          const newConfig = createGizmoScaleAxis(item)
          gizmoScale.Z.push(newConfig)
        })
      }
      if (uiScaleConfig.xy) {
        gizmoScale.XY = [
      ...
      if (uiScaleConfig.xyz) {
        gizmoScale.XYZ = [[new Mesh(new BoxGeometry(0.1, 0.1, 0.1), matWhiteTransparent.clone())]]
      }
    } else {
      gizmoScale = {
        X: [
    ...
    const uiPickerScaleConfig = uiGzConfig?.pickerScale
    let pickerScale = {}
    if (uiPickerScaleConfig) {
      if (uiPickerScaleConfig.x) {
        pickerScale.X = [
          [
            new Mesh(new CylinderGeometry(0.2, 0, 0.6, 4), matInvisible),
            [0.3, 0, 0],
      ...
      if (uiPickerScaleConfig.xy) {
        pickerScale.XY = [
          [new Mesh(new BoxGeometry(0.2, 0.2, 0.01), matInvisible), [0.15, 0.15, 0]],
        ]
      }
      if (uiPickerScaleConfig.yz) {
        pickerScale.YZ = [
          [
            new Mesh(new BoxGeometry(0.2, 0.2, 0.01), matInvisible),
            [0, 0.15, 0.15],
      ...
      if (uiPickerScaleConfig.xyz) {
        pickerScale.XYZ = [[new Mesh(new BoxGeometry(0.2, 0.2, 0.2), matInvisible), [0, 0, 0]]]
      }
    } else {
      pickerScale = {
        X: [
